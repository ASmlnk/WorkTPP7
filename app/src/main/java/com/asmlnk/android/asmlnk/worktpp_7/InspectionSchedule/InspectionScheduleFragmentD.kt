...
class InspectionScheduleFragmentD: Fragment() {

    val nameFragment = InSc.DEM.get
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        inspectionScheduleViewModel.inspectionScheduleLiveDataD.observe(viewLifecycleOwner) {
            updateUI(it)
        }
        //inspectionScheduleViewModel.getInspectorSchedule ( InSc.NIGHT.get, InSc.DEM.get )

        arguments?.takeIf { it.containsKey(ARG_OBJECT) }?.apply {
            val workingShift: String = getSerializable ( ARG_OBJECT ).toString()
            inspectionScheduleViewModel.getInspectorSchedule ( workingShift, InSc.DEM.get )
        }
